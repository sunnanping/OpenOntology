...
| REQ-013 | 标准元数据管理 | 实现标准元数据管理模块，添加缓存能力 | 高 | ✅ 已完成 |
...
| 标准元数据管理 | 实现metadata-module模块，添加缓存能力 | v0.1.16 | ✅ 已验证 |
...
1. **MetadataService**
   - 位置：`ontology-service/metadata-module/src/main/java/com/by/ontology/ontologyservice/metadatamodule/service/MetadataService.java`
   - 功能：
     - 使用OWLAPI获取标准的OWL属性、RDFS属性、RDF属性和数据类型
     - 提供缓存机制，使用Spring Cache缓存标准元数据
...
2. **MetadataController**
   - 位置：`ontology-service/metadata-module/src/main/java/com/by/ontology/ontologyservice/metadatamodule/controller/MetadataController.java`
   - 功能：
     - 提供RESTful API端点，支持获取标准元数据
     - 端点包括：
       - GET /api/metadata/owl/properties - 获取标准OWL属性
       - GET /api/metadata/rdfs/properties - 获取标准RDFS属性
       - GET /api/metadata/rdf/properties - 获取标准RDF属性
       - GET /api/metadata/datatypes - 获取标准数据类型

3. **Maven配置**
   - 位置：`ontology-service/metadata-module/pom.xml`
   - 功能：添加spring-boot-starter-cache依赖，支持缓存功能
...
1. **前端缓存策略**
   - 位置：`frontend/src/components/modules/ClassEditorPanel.vue`
   - 功能：
     - 使用localStorage缓存标准元数据
     - 实现缓存降级策略，当API不可用时使用缓存数据或默认值
     - 优化数据加载流程，提高页面响应速度
...
2. **数据加载逻辑**
   ...
   - 功能：
     - 从后端API获取标准元数据并缓存
     - 当API调用失败时，使用前端默认值
     - 合并标准OWL属性和ObjectProperty数据

3. **缓存机制**
   - 位置：`frontend/src/components/modules/ClassEditorPanel.vue`
   - 功能：
     - 缓存relationshipProperties数据到localStorage
     - 缓存classOptions、individualOptions、dataTypeOptions等数据
     - 实现数据过期和更新机制
...
| metadata-module | 标准元数据管理模块 | eureka-server | application.yml |
...
| metadata-module | http://localhost:8082/api/metadata/** | 标准元数据管理相关接口 |
...
#### 6.2.11 标准元数据服务测试
- 测试获取标准OWL属性：GET /api/metadata/owl/properties
- 测试获取标准RDFS属性：GET /api/metadata/rdfs/properties
- 测试获取标准RDF属性：GET /api/metadata/rdf/properties
- 测试获取标准数据类型：GET /api/metadata/datatypes
